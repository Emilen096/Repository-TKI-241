...
template<typename T>
Matrix<T>::Matrix(std::initializer_list<T> init) : size_(init.size()), capacity_(init.size()), gen_(nullptr) {
    if (capacity_ > 0) {
...
template<typename T>
Matrix<T>::Matrix(const Matrix& other) : size_(other.size_), capacity_(other.capacity_), gen_(nullptr) {
    if (capacity_ > 0) {
        data_ = new T[capacity_];
...
template<typename T>
Matrix<T>::Matrix(Matrix&& other) noexcept : data_(other.data_), size_(other.size_), capacity_(other.capacity_), gen_(std::move(other.gen_)) {
    other.data_ = nullptr;
    other.size_ = 0;
    other.capacity_ = 0;
}
...
Matrix<T>& Matrix<T>::operator=(const Matrix& other) {
    if (this != &other) {
        ...
        gen_ = nullptr; // gen_ не копируется
        if (capacity_ > 0) {
            data_ = new T[capacity_];
            for (std::size_t i = 0; i < size_; ++i) {
                data_[i] = other.data_[i];
...
template<typename T>
void Matrix<T>::insert(std::size_t index, const T& value) {
    if (index > size_) {
...
void Matrix<T>::fill(std::size_t n) {
    this->clear();
    for (std::size_t i = 0; i < n; ++i) {
        this->push_back(static_cast<T>(gen_->generate()));
    }
}
